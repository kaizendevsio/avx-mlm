...
if (isset($_POST['package_name'])){$postData->package_name = cleanString($_POST['package_name']);}

$allusers_OBJ = array();

if ($postData->intent != '')
{
    if($postData->intent == 'single'){
        $response = '';
        $package = new packages;


        $allusers_OBJ = $package->getPackageAmount($package->getPackageID($postData->package_name));
        $response = '200';
    }
...
$resultData->result = $allusers_OBJ;
